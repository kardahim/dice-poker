...
//  dice poker
//
//  Created by Kardahim on 03/01/2024.
//
...
struct RankingOfHands: View {
    var body: some View {
        VStack {
            List {
                Section(header: VStack{Text("Ranking of Hands").font(.title).padding(.horizontal)
                    Text("From the lowest to the highest")
                },
                        footer: Text("In the case of the same dice arrangement for both players at the same time, higher dice values win")) {
